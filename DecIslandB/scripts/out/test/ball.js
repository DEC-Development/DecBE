...
function ball_main() {
    ...
    for (let i = 0; i < 50; i++) {
        ...
        for (let x = 0; x <= i; x++) {
            for (let z = 0; z <= x; z++) {
                for (let y = 0; y <= i; y++) {
                    let tmpV = new Vector3(x, y, z);
                    if (i < tmpV.len() && tmpV.len() <= i + 1) {
                        curArr.push(x | y << 8 | z << 16);
                        // console.log(tmpV)
                    }
                }
            }
    ...
    for (let blss of arr) {
        for (let bls of blss) {
            // console.log(`${bls & 0xff}, ${(bls >> 8) & 0xff}, ${(bls >> 16) & 0xff}`)
        }
    }
    const ncp = require('copy-paste');
    ncp.copy(JSON.stringify(arr), function () {
    });
}
ball_main();
