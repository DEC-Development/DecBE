...
export class KDNode {
    constructor(point, axis, parent = undefined) {
        this.point = point;
        this.left = undefined;
...
export default class KDTree {
    ...
    insert(point) {
        ...
        else if (this.maxDepth > 48 && this.getNodes().size < 2048) {
            this.rebuild();
        }
    }
    /**
    ...
    */
    _chooseSplitDimension(points, depth) {
        let maxVariance = -1;
    ...
    */
    _calculateVariance(points, dimension) {
    ...
    nearestNeighborBBF(queryPoint) {
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            if (bestNode !== undefined && dist > manhattanDistance(queryPoint, bestNode.point) + 2)
                break; // 如果当前节点距离大于已找到的最佳距离，则停止搜索
        ...
        return bestNode === null || bestNode === void 0 ? void 0 : bestNode.point;
    }
    /**
     ...
     * @returns {KDPoint | undefined} - The nearest neighbor or undefined if not found.
     */
    _nearestNeighbor(node, queryPoint) {
        if (node === undefined)
            return undefined;
        ...
        if (!nearestInNext || manhattanDistance(nearestInNext, queryPoint) > dist)
        ...
        if (Math.abs(node.point.coordinates[axis] - queryPoint.coordinates[axis]) < curDist) {
            ...
            if (!nearestInOther || manhattanDistance(nearestInOther, queryPoint) > curDist)
     ...
     */
    _rangeSearch(node, center, radius, result) {
        if (node === undefined)
            return;
        const axis = node.axis;
        const dist = Math.abs(node.point.coordinates[axis] - center.coordinates[axis]);
        if (dist <= radius) {
            if (manhattanDistance(node.point, center) <= radius)
