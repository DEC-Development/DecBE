...
function runComMult(en, commands) {
    commands.forEach(c => {
...
function findEleInArr(aim_e, arr) {
    let r = [];
    if (arr) {
...
world.events.tick.subscribe(e => {
    ...
    for (const p of Array.from(world.getPlayers())) {
        //蓝魔法卷轴
        let c = p.getComponent("minecraft:inventory").container;
...
world.events.itemUse.subscribe(e => {
    ...
    if (e.item.typeId == "dec:magic_scroll_blue") {
        let c = e.source.getComponent("minecraft:inventory").container;
        taskUi(e.source, c.getItem(e.source.selectedSlot));
    }
});
